...
export function FamilyPanel({ onSignIn, onSignUp }: { onSignIn: () => void; onSignUp: () => void }) {
  ...
  const button = (label: string, action: () => void) => <Pressable accessibilityRole="button" disabled={busy} onPress={action} style={[st.settingsButton, { backgroundColor: "#E4F5EB", borderWidth: 1, borderColor: "#BED9C8" }]}><Text style={[st.settingsButtonSecondaryText, label === "Leave family" && { color: "#A83939" }]}>{label}</Text></Pressable>;
  ...
  return <SettingsSection label="Family">
    <View style={{ padding: 16, gap: 12 }}>
      <Pressable accessibilityRole="button" accessibilityState={{ expanded: expanded }} onPress={() => setExpanded(!expanded)}>
      ...
      <AppSheet title="Family" visible={expanded} busy={busy} onClose={() => setExpanded(false)}>
        ...
        {!family.userId ? <>
          ...
          {button("Create account", () => { setExpanded(false); onSignUp(); })}{button("Sign in", () => { setExpanded(false); onSignIn(); })}
        </> : !family.ready ? button("Refresh family", () => void family.refresh()) : family.family ? <>
          <Text style={st.shoppingSectionTitle}>Members</Text>
          ...
          {family.family.owner_id === family.userId ? <>
            <Text style={st.shoppingSectionTitle}>Invitations</Text>
          ...
          <Text style={st.shoppingSectionTitle}>Shared food</Text>
          {button("Move my personal food to family", () => confirm("Share your existing food?", "Your personal My Freezer items will move into this family's shared inventory. Your personal Cart stays separate.", async () => { await familyAction("import_freezer"); setMessage("Your food is now shared with your family."); await family.refresh(); }))}
          <Text style={st.shoppingSectionTitle}>Membership</Text>
          ...
          <Text style={st.settingsHelp}>Your personal items stay private until you choose to share them.</Text>
