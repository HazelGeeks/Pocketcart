...
export default function useAdminWorkspaceQueries(state: AdminWorkspaceState) {
  ...
  const productAliasesQuery = useAdminProductAliasesQuery(enabled && overviewOrProducts);
  ...
  const mutations = {
    ...
    // These actions explicitly refresh once after their full save/import workflow completes.
    // Suppressing per-row invalidation prevents bulk work from repeatedly downloading all prices.
    createProduct: useCreateAdminProductMutation({ invalidateOnSuccess: false }),
    createIdentityReview: useCreateProductIdentityReviewMutation(),
    updateProduct: useUpdateAdminProductMutation({ invalidateOnSuccess: false }),
    createPrice: useCreateAdminPriceEntryMutation({ invalidateOnSuccess: false }),
    createAuditLog: useCreateAdminAuditLogMutation(),
    updatePrice: useUpdateAdminPriceEntryMutation({ invalidateOnSuccess: false }),
    deletePrice: useDeleteAdminPriceEntryMutation({ invalidateOnSuccess: false }),
    createStore: useCreateAdminStoreMutation(),
    deleteStore: useDeleteAdminStoreMutation(),
    updateStore: useUpdateAdminStoreMutation(),
    deleteProduct: useDeleteAdminProductMutation({ invalidateOnSuccess: false }),
  ...
  const products = React.useMemo<AdminProduct[]>(
    () => productsQuery.data ?? [],
    [productsQuery.data],
  );
  const productAliases = React.useMemo<AdminProductAlias[]>(
    () => productAliasesQuery.data ?? [],
    [productAliasesQuery.data],
  );
  ...
  const reviews = React.useMemo<AdminProductIdentityReview[]>(
    () => reviewsQuery.data ?? [],
    [reviewsQuery.data],
  );
  ...
  const syncQueryErrorNotice = React.useCallback(
    (errors: string[], clearOtherNotice = false) => {
      if (errors.length) {
  ...
  const loadAll = React.useCallback(
    async (keepNotice = false) => {
      ...
      if (activeMenu === "overview" || activeMenu === "stores")
        requests.push(auditLogsQuery.refetch());
  ...
  React.useEffect(() => {
    const errors = [
      ...
      adminAccessQuery.error,
      usersQuery.error,
      productsQuery.error,
      productAliasesQuery.error,
      storesQuery.error,
      pricesQuery.error,
  ...
  return {
    ...
    loading: {
      ...
      auth:
        adminUserQuery.isLoading ||
        adminAccessQuery.isLoading ||
        adminAccessQuery.isFetching ||
        mutations.signIn.isPending ||
        mutations.signOut.isPending,
    },
    errors: {
      products:
        productsQuery.error instanceof Error
          ? productsQuery.error.message
          : productAliasesQuery.error instanceof Error
            ? productAliasesQuery.error.message
            : null,
    },
    queries: { productsQuery, productAliasesQuery, pricesQuery, auditLogsQuery, reviewsQuery },
    mutations,
    loadAll,
    handleRefresh,
  };
