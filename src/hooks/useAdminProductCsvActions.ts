...
export default function useAdminProductCsvActions(params: Params) {
  const [productCsvPreview, setProductCsvPreview] =
    React.useState<ProductCsvImportPreview | null>(null);
  const [productCsvReport, setProductCsvReport] =
    React.useState<ProductCsvImportReport | null>(null);
  ...
  const handleExportProductsCsv = React.useCallback((selectedProducts: AdminProduct[]) => {
    ...
    const error = downloadCsvFile("products", productsToCsv(selectedProducts, params.productPriceStats));
  ...
  const handleImportProductsCsv = React.useCallback(() => {
    ...
    input.onchange = () => {
      ...
      if (!selected) return;
      void selected.text().then((csvText: string) => {
  ...
  const handleDownloadProductCsvReviewRows = React.useCallback(() => {
    ...
    const error = downloadCsvFile("product-import-review", productCsvReviewRowsToCsv(productCsvPreview));
  ...
  const handleDownloadProductCsvReport = React.useCallback(() => {
    ...
    const error = downloadCsvFile("product-import-report", productCsvImportReportToCsv(productCsvReport));
    if (error) params.setNotice(error);
