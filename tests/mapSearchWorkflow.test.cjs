...
test('choosing a location moves the map and recalculates distance without filtering by address label', async () => {
  ...
  assert.equal(map.locationResults.length, 1);
  map.selectLocation(map.locationResults[0]); map = await h.render();
...
test('stale location responses cannot override a newer search or a clear action', async () => {
  ...
  pending[1]([point]); map = await h.render(); assert.equal(map.locationResults[0].label, point.label);
  pending[0]([{ ...point, label: 'Old' }]); map = await h.render(); assert.equal(map.locationResults[0].label, point.label);
...
test('clearing a saved postal search stays cleared across map visits', async () => {
  ...
  let map = await h.render(); assert.equal(map.query, 'V6X1A1');
