...
exports.navigationHookHarness = function navigationHookHarness() {
  ...
  const native = {
    ...
    Animated: {
      ...
      spring: (value) => ({ start() { value.setValue(0); } }),
      timing: () => ({ start(fn) { fn({ finished: true }); } }),
    },
    PanResponder: { create: (handlers) => ({ panHandlers: handlers }) },
  };
  vm.runInNewContext(ts.transpileModule(fs.readFileSync('src/hooks/useNativeBackNavigation.ts', 'utf8'), {
    compilerOptions: { module: ts.ModuleKind.CommonJS, target: ts.ScriptTarget.ES2022, esModuleInterop: true },
  }).outputText, { exports: mod.exports, require(name) {
    if (name === 'react') return react;
    if (name === 'react-native') return native;
    if (name.includes('nativeBackNavigation')) return require('../../.tmp-tests/utils/nativeBackNavigation.js');
    throw Error(name);
  } });
  return {
